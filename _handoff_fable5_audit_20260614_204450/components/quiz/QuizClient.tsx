...
export default function QuizClient() {
  ...
  useEffect(() => {
    ...
    if (existing && existing.questionIds.length > 0 && sessionMatchesMode(existing, mode, subject)) {
      setSession(existing);
      return;
    }

    const next = buildSessionFromMode(mode, subject);
    ...
    <main style={pageStyle}>
      ...
      {!isExamMode && revealed && (
        <section style={explanationStyle}>
          ...
          <p style={explanationTextStyle}>{currentQuestion.explanation}</p>
